...
# Load .env if present (for persistent API keys)
try:
...
try:
	...
	_gemini_ready = bool(Config.GEMINI_API_KEY)
except Exception:
	genai = None
	_gemini_ready = False
...
def mood_log():
	...
	mood_entries = db_manager.get_mood_history(user_id=user_id, days=days, emotion=emotion)
	
	return render_template('mood_log.html', mood_entries=mood_entries, days=days, emotion=emotion)

@app.route('/mood_log_partial')
@login_required
def mood_log_partial():
	user_id = session.get('user_id')
	days = request.args.get('days', 30, type=int)
	emotion = request.args.get('emotion') or None
	mood_entries = db_manager.get_mood_history(user_id=user_id, days=days, emotion=emotion)
	return render_template('partials/mood_table.html', mood_entries=mood_entries)
...
def suggestions():
	...
	# Try to load existing suggestions for this mood from DB (to get IDs)
	db_suggestions = db_manager.get_suggestions_by_mood(recent_mood['id']) if 'id' in recent_mood else []
	if not db_suggestions:
		# Generate and persist if none exist yet
		generated = suggestion_engine.get_suggestions(emotion)
		db_manager.save_suggestions(recent_mood['id'], generated)
		db_suggestions = db_manager.get_suggestions_by_mood(recent_mood['id'])

	# Normalize for template: use keys 'id', 'type', 'content'
	suggestions_norm = [{
		'id': s.get('id'),
		'type': s.get('suggestion_type') or s.get('type'),
		'content': s.get('content'),
		'helpful_rating': s.get('helpful_rating'),
		'used': s.get('used')
...
def use_suggestion():
    user_id = session.get('user_id')
    suggestion_id = request.json.get('suggestion_id')
    success = db_manager.mark_suggestion_used(user_id, suggestion_id)
    return jsonify({'success': success})
...
@login_required
def ai_chat():
	if not _gemini_ready or genai is None:
		return jsonify({'success': False, 'error': 'Gemini not configured on server'}), 500

	data = request.get_json(silent=True) or {}
	user_message = (data.get('message') or '').strip()
	emotion = (data.get('emotion') or '').strip()  # optional context
	...
	try:
		system_preamble = (
			"You are a supportive wellbeing assistant for a mood-tracking app. "
			"Be empathetic and concise. Offer 3-5 actionable, safe suggestions tailored to the user's mood. "
			"If the user indicates severe distress or crisis, gently encourage contacting local helplines or trusted people. "
			"Avoid medical diagnoses. Use short paragraphs and bullet points when listing actions."
		)

		model = genai.GenerativeModel('gemini-1.5-flash')
		prompt = f"Mood context: {emotion or 'Unknown'}\nUser: {user_message}\nRespond supportively."
		...
		text = text or 'Sorry, I could not generate a response right now.'
